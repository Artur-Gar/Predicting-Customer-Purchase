...
class BaseKNNImputer:
    ...
    # this function transforms column: if we already imputed values, then it just does OHE + standartization. If not - imputes either median ro most frequent value  
    def transform_columns(self, df_fix, visited_num_columns, unvisited_num_columns, visited_cat_columns, unvisited_cat_columns):  
        ...
        ct.fit_transform(df_fix) 

        return ct ###

    # will use either classifier or regressor. In general, can implement different models, but we focus on KNN
    ...
    # Main part. For selected column we try to impute values with use of regression/classifier models (Knn here). All other columns with missing values are filled at transformation stage
    def impute(self, column_to_impute, df_mask, visited_num_columns, unvisited_num_columns, visited_cat_columns, unvisited_cat_columns, missing_indices):
        ...
        # Use indices for initially missing values
        ...
        # Transform columns for both train and target sets
        transformer = self.transform_columns(X, visited_num_columns, unvisited_num_columns, visited_cat_columns, unvisited_cat_columns)
        X = pd.DataFrame(transformer.transform(X))
        X_target = pd.DataFrame(transformer.transform(X_target))


        # Want to adjust number of neigbors. But CV is slow. Do simple train_test_split
...
class IterativeKNNImputer:
    ...
    def impute(self, df_mask, numerical_cols, categorical_cols):
        ...
        # Iterate further to recycle: KNN performs better than trivial imputation with median/mode. Therefore, each newly imputed column will improve the quality of all subsequent imputations
        # After filling all missing values, we start reimputing them again, leveraging the previously recycled missing values to enhance quality
        # Number of epochs - number of imputation cycles we have
        for epoch in range(1, self.epochs):
            ...
            while len(all_columns) > 0:
                ...
                # update df and remove column from the list for this epoch
                df_imputed = knn_imputer.impute(column_to_impute, df_imputed, visited_num_columns, unvisited_num_columns, visited_cat_columns, unvisited_cat_columns, missing_indices)
